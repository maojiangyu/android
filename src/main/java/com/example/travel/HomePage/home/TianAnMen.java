...
public class TianAnMen extends Activity {

    private ViewPager guide;
    //引导页图片ID数组
    private int[] ImageIds = new int[]{
            R.drawable.tiananmen1, R.drawable.tiananmen2,R.drawable.tiananmen3
    };
    //imageView的集合
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.tiananmen);

        initViews();//初始化界面控件
        initData();//先初始化数据,然后再设置数据
        guide.setAdapter(new TianAnMen.GuideAdapter());//设置数据

        initTxs();//下面的初始化方法
        RecyclerView recyclerView = findViewById(R.id.TianAn_recyclerView2);//找到RecyclerView控件
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);//布局管理器
        recyclerView.setLayoutManager(layoutManager);
        NearAdapter adapter = new NearAdapter(txList, TianAnMen.this, new NearAdapter.OnItemClickListener() {
            //@Override
    ...
    private void init(){
        mBtn = findViewById(R.id.TianAnMapBtn);
        mBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(TianAnMen.this,TianAnMenMap.class));
            }
        });
    }

    private void initViews() {
        guide = (ViewPager) findViewById(R.id.guide_tianan);
        return1 = findViewById(R.id.btn_tianan_return);


        return1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                TianAnMen.this.finish();
            }
        });
        //对ViewPager页面滑动的监听
    ...
    public void OnClickToTianAnInformation(View view){
        startActivity(new Intent(TianAnMen.this,TianAnMenInformation.class));
    }
