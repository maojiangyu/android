...
public class ChangCheng extends Activity {
    private ViewPager guide;
    //引导页图片ID数组
    private int[] ImageIds = new int[]{
            R.drawable.changchen1,R.drawable.changchen2,R.drawable.changchen3
    };
    //imageView的集合
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.changcheng);

        initViews();//初始化界面控件
        initData();//先初始化数据,然后再设置数据
        guide.setAdapter(new ChangCheng.GuideAdapter());//设置数据

        initTxs();//下面的初始化方法
        RecyclerView recyclerView = findViewById(R.id.changcheng_recyclerView2);//找到RecyclerView控件
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);//布局管理器
        recyclerView.setLayoutManager(layoutManager);
        NearAdapter adapter = new NearAdapter(txList, ChangCheng.this, new NearAdapter.OnItemClickListener() {
            //@Override
    ...
    private void init(){
        mBtn = findViewById(R.id.changchengMapBtn);
        mBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(ChangCheng.this, ChangChengMap.class));
            }
        });
    }

    private void initViews() {
        guide = (ViewPager) findViewById(R.id.guide_changcheng);
        return1 = findViewById(R.id.btn_changcheng_return);


        return1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ChangCheng.this.finish();
            }
        });
        //对ViewPager页面滑动的监听
    ...
    public void OnClickToChangChengInformation(View view){
        startActivity(new Intent(ChangCheng.this,ChangChengInformation.class));
    }
}
