...
public class GuGong extends Activity {
    private ViewPager guide;
    //引导页图片ID数组
    private int[] ImageIds = new int[]{
            R.drawable.gugong1,R.drawable.gugong2,R.drawable.gugong3
    };
    //imageView的集合
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.gugong);

        initViews();//初始化界面控件
        initData();//先初始化数据,然后再设置数据
        guide.setAdapter(new GuGong.GuideAdapter());//设置数据

        initTxs();//下面的初始化方法
        RecyclerView recyclerView = findViewById(R.id.GuGong_recyclerView2);//找到RecyclerView控件
        LinearLayoutManager layoutManager = new LinearLayoutManager(this);//布局管理器
        recyclerView.setLayoutManager(layoutManager);
        NearAdapter adapter = new NearAdapter(txList, GuGong.this, new NearAdapter.OnItemClickListener() {
            //@Override
    ...
    private void init(){
        mBtn = findViewById(R.id.GuGongMapBtn);
        mBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(GuGong.this,GuGongMap.class));
            }
        });
    }

    private void initViews() {
        guide = (ViewPager) findViewById(R.id.guide_GuGong);
        return1 = findViewById(R.id.btn_GuGong_return);
    ...
    public void OnClickToGuGongInformation(View view){
        startActivity(new Intent(GuGong.this,GuGongInformation.class));
    }
}
